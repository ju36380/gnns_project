...
function fn_getAttachementFile(){
	let url = "/board/getAttachementFile";
	$.ajax({
		type: "POST",
		url  : url,
...
function fn_createFileArea(data){
	...
	if(data.attchementFile=="" || data.attchementFile==null){
		...
		fileHtml += "<a href='/file/download?filePath=board&fileSeq="+getUrlParams()+"'class='txt_down "+exeType+"'>"+data.attchementFile+"</a>";
	}
	fileHtml +="</td>";
	fileHtml +="</tr>";
...
function fn_createBtn(){
	let buttonHtml ="";
	buttonHtml += "<a href='/board/boardAdminReply?boardSeq="+$('#boardSeq').val()+"&page="+$('#page').val()+"&searchType="+$('#searchType').val()+"&keyword="+$('#keyword').val()+"' class='btn_type_01 gray list'>답변</a>";
	buttonHtml += "&nbsp;";
	buttonHtml += "<a href='/board/boardAdminList?page="+$('#page').val()+"&searchType="+$('#searchType').val()+"&keyword="+$('#keyword').val()+"' class='btn_type_01 gray list'>목록</a>";
	$("#divBtn").append(buttonHtml);
};

function fn_boardList(){
	let frm = document.boardListForm;
	let searchType = $("#searchType").val();
	let keyword  = $("#keyword").val();
	let page  = $("#page").val();
	frm.action="/board/boardAdminList?page="+page+"&searchType="+searchType+"&keyword="+keyword;
	frm.submit();
}
...
function boardDel(){
	if(confirm("삭제하시겠습니까?") == true){
		var seq=$("#seqNum").val();
		window.location="/board/boardAdminDelete?boardSeq="+seq;
	}else{
		return false;
