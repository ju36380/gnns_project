...
function fn_getBoardList(page){
	...
	$.ajax({
		...
		success: function(resultData) {
			fn_createList(resultData.boardList, searchType, keyword, page);
	...
	let listHtml="";
		...
		if(data.length>0){
			for(let i=0; i<data.length; i++){
				...
				if(data[i].reLevel>0){
					listHtml += "<li class='col_title'><a href='/board/boardView?boardSeq="+data[i].boardSeq+"&searchType="+arg2+"&keyword="+arg3+"&page="+arg4+"' class='txt_01 reple'>"+data[i].subject+"</a></li>";
				}else{
					listHtml += "<li class='col_title'><a href='/board/boardView?boardSeq="+data[i].boardSeq+"&searchType="+arg2+"&keyword="+arg3+"&page="+arg4+"' class='txt_01'>"+data[i].subject+"</a></li>";
				}
