...
function getUrlParams() {
	...
	return params.pdsSeq;
};


function fn_getAttachementFile(){
	let url = "/pds/getAttachementFile";
	$.ajax({
		type: "POST",
		url  : url,
		data :  {pdsSeq : getUrlParams()},
...
function fn_createFileArea(data){
	let contentsHtml = "";
	contentsHtml+=$("#contentsTest").text();
	...
	if(data.attchementFile=="" || data.attchementFile==null){
		...
		fileHtml += "<a href='/file/download?filePath=pds&fileSeq="+getUrlParams()+"'class='txt_down "+exeType+"'>"+data.attchementFile+"</a>";
	}
	fileHtml +="</td>";
	fileHtml +="</tr>";
...
function fn_createBtn(){
	let buttonHtml ="";
	buttonHtml += "<a href='/pds/pdsAdminList?page="+$('#page').val()+"&searchType="+$('#searchType').val()+"&keyword="+$('#keyword').val()+"' class='btn_type_01 gray list'>목록</a>";
	$("#divBtn").append(buttonHtml);
};

function fn_pdsList(){
	let frm = document.pdsListForm;
	let searchType = $("#searchType").val();
	let keyword  = $("#keyword").val();
	let page  = $("#page").val();
	frm.action="/pds/pdsAdminList?page="+page+"&searchType="+searchType+"&keyword="+keyword;
	frm.submit();
}
...
function pdsDel(){
	if(confirm("삭제하시겠습니까?") == true){
		var seq=$("#seqNum").val();
		window.location="/pds/pdsAdminDelete?pdsSeq="+seq;
	}else{
		return false;
