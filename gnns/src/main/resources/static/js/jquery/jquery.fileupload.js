...
(function ($) {
    ...
    var FileUpload = function (dropZone) {
        ...
        uploadFile = function (files, index, xhr, settings) {
            ...
            if(settings.url == ""){
            	xhr.open(settings.method, $("#"+settings.id).attr("action"), true);
            }
            else{
        ...
        legacyUpload = function (input, settings) {
        	if(settings.url == ""){
        		uploadForm.attr('action',$("#"+settings.id).attr("action")).attr('target', iframe.attr('name'));
        	}
        	else{
            ...
            if($("#user_id_file").attr('name') != undefined){
            	$("#user_id_file").val(datagrid1.getTextMatrix(datagrid1.selectedRow(0), datagrid1.getColRef('user_id')));
            }
            if($("#regNumFile").attr('name') != undefined){
            	$("#regNumFile").val(datagrid1.getTextMatrix(datagrid1.selectedRow(0), datagrid1.getColRef('regNum')));
            }
        ...
        this.dragOver = function (e) {
            ...
            var dataTransfer = e.originalEvent.dataTransfer;
            if (dataTransfer) {
                //ataTransfer.dropEffect = dataTransfer.effectAllowed = 'copy';
            	return false;
            }
            e.preventDefault();
            e.stopPropagation();
        ...
        this.inputChange = function (e) {
            ...
            if (e.target.files && isXHRUploadCapable()) {
            	...
            	if(e.target.value.lastIndexOf('.') > 0){
            		ext = e.target.value.substring(e.target.value.lastIndexOf('.')+1,e.target.value.length).toLowerCase();
            	}
            	var flag = false;
            	if(settings.fileext.length == 0){
            		flag = true;
            	}
            	for(var i=0;i<settings.fileext.length;i++){
            		if(settings.fileext[i] == ext){
            			flag = true;
            		}
            	}
            	if(!flag){
            		alert(settings.fileextmsg);
            		return;
            	}
            	if(settings.quest != null)
            	if(!confirm(settings.quest)){
            		return;
            	}
